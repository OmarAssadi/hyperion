...
public class NPCUpdateTask implements Task {
     ...
     */
    public NPCUpdateTask(final Player player) {
        this.player = player;
    }
    ...
    public void execute(final GameEngine context) {
         ...
         * NPCs.
         */
        final PacketBuilder packet = new PacketBuilder(65, Packet.Type.VARIABLE_SHORT);
         ...
         */
        for (final NPC npc : World.getWorld().getRegionManager().getLocalNpcs(player)) {
            /*
     ...
     * @param npc    The npc.
     */
    private void updateNPCMovement(final PacketBuilder packet, final NPC npc) {
        /*
     ...
     * @param npc    The npc.
     */
    private void updateNPC(final PacketBuilder packet, final NPC npc) {
        /*
         * Calculate the mask.
         */
        int mask = 0;
        ...
        if (flags.get(UpdateFlag.FACE_ENTITY)) {
            final Entity entity = npc.getInteractingEntity();
        ...
        if (flags.get(UpdateFlag.FACE_COORDINATE)) {
            final Location loc = npc.getFaceLocation();
            if (loc == null) {
                packet.putLEShort(0);
                packet.putLEShort(0);
            } else {
     ...
     */
    private void addNewNPC(final PacketBuilder packet, final NPC npc) {
        /*
         ...
         */
        final int yPos = npc.getLocation().getY() - player.getLocation().getY();
        final int xPos = npc.getLocation().getX() - player.getLocation().getX();
