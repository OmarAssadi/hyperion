            ...
            if (command.equals("tele")) {
                ...
                if (args.length == 3) {
                    try {
                        ...
                        final int x = Integer.parseInt(args[1]) - player.getLocation().getX() + radius;
                        final int y = Integer.parseInt(args[2]) - player.getLocation().getY() + radius;

                        final TileMapBuilder bldr = new TileMapBuilder(player.getLocation(), radius);
                        final TileMap map = bldr.build();

                        final PathFinder pf = new AStarPathFinder();
                        final Path p = pf.findPath(player.getLocation(), radius, map, radius, radius, x, y);
                        ...
                        for (final Point p2 : p.getPoints()) {
