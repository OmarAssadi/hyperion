...
public class ItemDefinition {
     ...
     */
    private ItemDefinition(final int id, final String name, final String examine, final boolean noted, final boolean noteable, final boolean stackable, final int parentId, final int notedId, final boolean members, final int shopValue, final int highAlcValue, final int lowAlcValue) {
        this.id = id;
        this.name = name;
    ...
    public static void init() throws IOException {
        ...
        try (final RandomAccessFile raf = new RandomAccessFile("data/itemDefinitions.bin", "r")) {
            final ByteBuffer buffer = raf.getChannel().map(MapMode.READ_ONLY, 0, raf.length());
            final int count = buffer.getShort() & 0xFFFF;
            definitions = new ItemDefinition[count];
            for (int i = 0; i < count; i++) {
                final String name = Buffers.readString(buffer);
                final String examine = Buffers.readString(buffer);
                final boolean noted = buffer.get() == 1;
                int parentId = buffer.getShort() & 0xFFFF;
                if (parentId == 65535) {
                    parentId = -1;
                }
                final boolean noteable = buffer.get() == 1;
                int notedId = buffer.getShort() & 0xFFFF;
                if (notedId == 65535) {
                    notedId = -1;
                }
                final boolean stackable = buffer.get() == 1;
                final boolean members = buffer.get() == 1;
                final boolean prices = buffer.get() == 1;
                int shop = -1;
                int highAlc = -1;
                int lowAlc = -1;
                if (prices) {
