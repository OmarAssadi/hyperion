        ...
        switch (state) {
            case STATE_UPDATE:
                if (in.remaining() >= 4) {
                     ...
                     */
                    final int cacheId = in.get() & 0xFF;
                    final int fileId = ((in.get() & 0xFF) << 8) | (in.get() & 0xFF);
                    final int priority = in.get() & 0xFF;
            ...
            case STATE_LOGIN:
                if (in.remaining() >= 1) {
                     ...
                     * server.
                     */
                    @SuppressWarnings("unused") final int nameHash = in.get() & 0xFF;
            ...
            case STATE_CRYPTED:
                ...
                if (in.remaining() >= size) {
                     ...
                     */
                    final int version = in.getShort() & 0xFFFF;
                     ...
                     * memory version.
                     */
                    @SuppressWarnings("unused") final boolean lowMemoryVersion = (in.get() & 0xFF) == 1;
                     ...
                     */
                    final int reportedSize = in.get() & 0xFF;
                    ...
                    /*
                     * We now read the encrypted block opcode and
                     * check it is equal to 10.
                     */
                    final byte[] rsaPayload = new byte[encryptSize];
                    in.get(rsaPayload);
                    final IoBuffer rsaBuffer = IoBuffer.wrap(new BigInteger(rsaPayload).modPow(NetworkConstants.RSA_EXPONENT, NetworkConstants.RSA_MODULUS).toByteArray());
                    final int blockOpcode = rsaBuffer.get() & 0xFF;
                    if (blockOpcode != 10) {
                     ...
                     */
                    final String name = NameUtils.formatName(IoBufferUtils.getRS2String(rsaBuffer));
                    final String pass = IoBufferUtils.getRS2String(rsaBuffer);
                    ...
                    final ISAACCipher inCipher = new ISAACCipher(sessionKey);
                    for (int i = 0; i < 4; i++) {
                        sessionKey[i] += 50;
                    }
                    final ISAACCipher outCipher = new ISAACCipher(sessionKey);
                    ...
                    final PlayerDetails pd = new PlayerDetails(session, name, pass, uid, inCipher, outCipher);
